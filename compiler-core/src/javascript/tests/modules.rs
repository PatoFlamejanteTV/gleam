...
fn unqualified_fn_call() {
    assert_js!(
        ("rocket_ship", r#"public fn launch() { 1 }"#),
...
fn constant() {
    assert_js!(
        ("rocket_ship", r#"public const x = 1"#),
        r#"
import rocket_ship
...
fn nested_nested_fn_call() {
    assert_js!(
        ("one/two/three", r#"public fn go() { 1 }"#),
...
fn different_package_import() {
    assert_js!(
        ("other_package", "one", r#"public fn go() { 1 }"#),
        r#"import one
pub fn go() { one.go() }
"#,
    );
...
fn discarded_duplicate_import() {
    assert_js!(
        ("esa/rocket_ship", r#"public fn go() { 1 }"#),
        ("nasa/rocket_ship", r#"public fn go() { 1 }"#),
        r#"
...
fn constant_module_access_with_keyword() {
    assert_js!(
        ("rocket_ship", r#"public const class = 1"#),
        r#"
import rocket_ship
