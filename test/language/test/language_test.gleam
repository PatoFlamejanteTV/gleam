...
func int_tests() -> List(Test) {
  let basic_addition = operator_test("+", func(a, b) { a + b })
  let basic_subtraction = operator_test("-", func(a, b) { a - b })
  let basic_multiplication = operator_test("*", func(a, b) { a * b })
  [
...
func triplet(x x, y y, z z) {
  #(x, y, z)
}

func pipes_tests() -> List(Test) {
  [
    "pipe last"
      |> example(func() {
        let result =
...
func tail_call_optimisation_tests() -> List(Test) {
  [
    "10 million recursions doesn't overflow the stack"
      |> example(func() { assert_equal(Nil, count_down(from: 10_000_000)) }),
    ...
    "not recursion, the function is shadowed its argument"
      |> example(func() {
        assert_equal(function_shadowed_by_own_argument(func() { 1 }), 1)
      }),
  ]
}

func function_shadowed_by_own_argument(function_shadowed_by_own_argument) {
  function_shadowed_by_own_argument()
}

func count_down(from i) {
  case i {
    0 -> Nil
    _ -> count_down(i - 1)
  }
...
func constants_tests() -> List(Test) {
  [
...
func imported_custom_types_test() -> List(Test) {
  [
    equality_test(
...
func equality_test(name: String, left: a, right: a) {
  example(name, func() { assert_equal(left, right) })
}

func lazy_equality_test(name: String, left: func() -> a, right: a) {
  example(name, func() { assert_equal(left(), right) })
}
...
// same result
func clause_guard_tests() -> List(Test) {
...
func prelude_tests() -> List(Test) {
  [
    "gleam.Ok"
      |> example(func() { assert_equal(Ok(1), gleam.Ok(1)) }),
    "gleam.Error"
      |> example(func() { assert_equal(Error(1), gleam.Error(1)) }),
    "gleam.Nil"
      |> example(func() { assert_equal(Nil, gleam.Nil) }),
  ]
}

func alternative_patterns_tests() -> List(Test) {
  let int_one = make_int_zero() + 1
...
func multiple_case_subjects() -> List(Test) {
  [
    "wildcard"
      |> example(func() {
...
func equality_tests() -> List(Test) {
  [
    "[] == []"
      |> example(func() { assert_equal(True, [] == []) }),
    "[] == [0]"
      |> example(func() { assert_equal(False, [] == [0]) }),
    "[0] == []"
      |> example(func() { assert_equal(False, [0] == []) }),
    "[0] == [0]"
      |> example(func() { assert_equal(True, [0] == [0]) }),
    "[] != []"
      |> example(func() { assert_equal(False, [] != []) }),
    "[] != [0]"
      |> example(func() { assert_equal(True, [] != [0]) }),
    "[0] != []"
      |> example(func() { assert_equal(True, [0] != []) }),
    "[0] != [0]"
      |> example(func() { assert_equal(False, [0] != [0]) }),
    "0 == 0"
      |> example(func() { assert_equal(True, 0 == 0) }),
    "0 != 0"
      |> example(func() { assert_equal(False, 0 != 0) }),
    "1 == 0"
      |> example(func() { assert_equal(False, 1 == 0) }),
    "1 != 0"
      |> example(func() { assert_equal(True, 1 != 0) }),
    "1 == 1"
      |> example(func() { assert_equal(True, 1 == 1) }),
    "1 != 1"
      |> example(func() { assert_equal(False, 1 != 1) }),
    "<<>> == <<>>"
      |> example(func() { assert_equal(True, <<>> == <<>>) }),
    "<<>> != <<>>"
      |> example(func() { assert_equal(False, <<>> != <<>>) }),
    // Bit arrays
    "<<1, 2>> == <<1, 2>>"
      |> example(func() { assert_equal(True, <<1, 2>> == <<1, 2>>) }),
    "<<1, 2>> != <<1, 2>>"
      |> example(func() { assert_equal(False, <<1, 2>> != <<1, 2>>) }),
    "<<1, 2>> == <<2>>"
      |> example(func() { assert_equal(False, <<1, 2>> == <<2>>) }),
    "<<1, 2>> != <<2>>"
      |> example(func() { assert_equal(True, <<1, 2>> != <<2>>) }),
    // Records
    "Ok(1) == Ok(1)"
      |> example(func() { assert_equal(True, Ok(1) == Ok(1)) }),
    "Ok(1) != Ok(1)"
      |> example(func() { assert_equal(False, Ok(1) != Ok(1)) }),
    "Ok(2) == Ok(1)"
      |> example(func() { assert_equal(False, Ok(2) == Ok(1)) }),
    "Ok(2) != Ok(1)"
      |> example(func() { assert_equal(True, Ok(2) != Ok(1)) }),
    "Error(1) == Error(1)"
      |> example(func() { assert_equal(True, Error(1) == Error(1)) }),
    "Error(1) != Error(1)"
      |> example(func() { assert_equal(False, Error(1) != Error(1)) }),
    "Error(2) == Error(1)"
      |> example(func() { assert_equal(False, Error(2) == Error(1)) }),
    "Error(2) != Error(1)"
      |> example(func() { assert_equal(True, Error(2) != Error(1)) }),
  ]
}

func bit_array_tests() -> List(Test) {
  [
    ...
    "<<\"abc\":utf8>> == <<97, 98, 99>>"
      |> example(func() { assert_equal(True, <<"abc":utf8>> == <<97, 98, 99>>) }),
    "<<\"😀\":utf8>> == <<\"\u{1F600}\":utf8>>"
      |> example(func() {
        assert_equal(True, <<"😀":utf8>> == <<"\u{1F600}":utf8>>)
      }),
    "<<<<1>>:bit_array, 2>> == <<1, 2>>"
      |> example(func() { assert_equal(True, <<<<1>>:bits, 2>> == <<1, 2>>) }),
    "<<1>> == <<1:int>>"
      |> example(func() { assert_equal(True, <<1>> == <<1:int>>) }),
    "<<80_000:16>> == <<56, 128>>"
      |> example(func() { assert_equal(True, <<80_000:16>> == <<56, 128>>) }),
    "<<-80_000:16>> == <<199, 128>>"
      |> example(func() { assert_equal(True, <<-80_000:16>> == <<199, 128>>) }),
    ...
    "<<0, 0>> == <<0.0:float-16>>"
      |> example(func() { assert_equal(True, <<0, 0>> == <<0.0:float-16>>) }),
...
@target(erlang)
func bit_array_target_tests() -> List(Test) {
  [
    "<<60, 0>> == <<1.0:float-16>>"
      |> example(func() { assert_equal(True, <<60, 0>> == <<1.0:float-16>>) }),
...
func sized_bit_array_tests() -> List(Test) {
  [
    "<<257:size(8)>> == <<1>>"
      |> example(func() { assert_equal(True, <<257:size(8)>> == <<1>>) }),
    "let i = 257\n<<i:size(8)>> == <<1>>"
      |> example(func() {
        let i = 257
    ...
    "<<257:size(16)>> == <<1, 1>>"
      |> example(func() { assert_equal(True, <<257:size(16)>> == <<1, 1>>) }),
    "let i = 257\n<<i:size(16)>> == <<1, 1>>"
      |> example(func() {
        let i = 257
    ...
    "<<257:size(24)>> == <<1, 1>>"
      |> example(func() { assert_equal(True, <<257:size(24)>> == <<0, 1, 1>>) }),
    "let i = 257\n<<i:size(24)>> == <<1, 1>>"
      |> example(func() {
        let i = 257
    ...
    "<<256:size(-1)>> == <<>>"
      |> example(func() { assert_equal(True, <<>> == <<256:size(-1)>>) }),
    "let i = 256\n<<i:size(-1)>> == <<>>"
      |> example(func() {
        let i = 256
...
func unaligned_bit_array_expression_tests() -> List(Test) {
  [
    "<<0xFF:6, 0:2>> == <<0xFA>>"
      |> example(func() { assert_equal(True, <<0xFF:6, 0:2>> == <<0xFC>>) }),
...
func unaligned_bit_array_pattern_tests() -> List(Test) {
  [
...
func dynamic_size_bit_array_pattern_tests() -> List(Test) {
  [
    "let size = 8
...
func non_utf8_string_bit_array_tests() -> List(Test) {
  [
...
func list_spread_tests() -> List(Test) {
  [
    "[1, ..[]]"
      |> example(func() { assert_equal([1, ..[]], [1]) }),
    "[1, 2, ..[]]"
      |> example(func() { assert_equal([1, 2, ..[]], [1, 2]) }),
    "[1, 2, ..[3]]"
      |> example(func() { assert_equal([1, 2, ..[3]], [1, 2, 3]) }),
    "[1, 2, ..[3, 4]]"
      |> example(func() { assert_equal([1, 2, ..[3, 4]], [1, 2, 3, 4]) }),
  ]
}

func precedence_tests() -> List(Test) {
  [
    "1 + 2 * 3"
      |> example(func() { assert_equal(7, 1 + 2 * 3) }),
    "3 * 1 + 2"
      |> example(func() { assert_equal(5, 3 * 1 + 2) }),
    "{ 1 + 2 } * 3"
      |> example(func() { assert_equal(9, { 1 + 2 } * 3) }),
    "3 * { 1 + 2 }"
      |> example(func() { assert_equal(9, 3 * { 1 + 2 }) }),
    "1 + 2 * 3 + 4"
      |> example(func() { assert_equal(11, 1 + 2 * 3 + 4) }),
    "2 * 3 + 4 * 5"
      |> example(func() { assert_equal(26, 2 * 3 + 4 * 5) }),
    "2 * { 3 + 1 } / 2"
      |> example(func() { assert_equal(4, 2 * { 3 + 1 } / 2) }),
    "5 + 3 / 3 * 2 - 6 * 4"
      |> example(func() { assert_equal(-17, 5 + 3 / 3 * 2 - 6 * 4) }),
    "-5 + -3 / -3 * -2 - -6 * -4"
      |> example(func() { assert_equal(-31, -5 + -3 / -3 * -2 - -6 * -4) }),
    "a + b / c * d - e * f"
      |> example(func() {
        let a = 5
        let b = 3
...
type FnBox {
  FnBox(f: func(Int) -> Int)
}

func call_returned_function_tests() -> List(Test) {
  [
    "call record access"
      |> example(func() {
        let b = FnBox(f: func(x) { x })
...
func floats_tests() -> List(Test) {
  [
    "2.0 /. 2.0"
      |> example(func() { assert_equal(1.0, 2.0 /. 2.0) }),
    "2.0 /. 0.0"
      |> example(func() { assert_equal(0.0, 2.0 /. 0.0) }),
  ]
}

func ints_tests() -> List(Test) {
  [
    "hex int"
      |> example(func() { assert_equal(15, 0xF) }),
    "octal int"
      |> example(func() { assert_equal(15, 0o17) }),
    "binary int"
      |> example(func() { assert_equal(15, 0b00001111) }),
    "1-1 should lex as 1 - 1"
      |> example(func() { assert_equal(0, 1 - 1) }),
    ...
    "1- 1 should lex as 1 - 1"
      |> example(func() { assert_equal(0, 1 - 1) }),
    "1 / 1"
      |> example(func() { assert_equal(1, 1 / 1) }),
    "1 / 0"
      |> example(func() { assert_equal(0, 1 / 0) }),
    "3 / 2"
      |> example(func() { assert_equal(1, 3 / 2) }),
    "3 / 0"
      |> example(func() { assert_equal(0, 3 / 0) }),
  ]
}

func remainder_tests() -> List(Test) {
  [
    "1 % 1"
      |> example(func() { assert_equal(0, 1 % 1) }),
    "1 % 0"
    ...
    "3 % -0"
      |> example(func() { assert_equal(0, 3 % -0) }),
    "-13 % 3"
      |> example(func() { assert_equal(-1, -13 % 3) }),
    "13 % -3"
      |> example(func() { assert_equal(1, 13 % -3) }),
    "-13 % -3"
      |> example(func() { assert_equal(-1, -13 % -3) }),
...
func mod_with_numbers_tests() -> List(Test) {
  [
...
func record_access_tests() {
  let person = Person(name: "Quinn", age: 27, country: "Canada")
  [
    "record access 1"
      |> example(func() { assert_equal(person.name, "Quinn") }),
    "record access 2"
      |> example(func() { assert_equal(person.age, 27) }),
    // https://github.com/gleam-lang/gleam/issues/1093
    "contextual info for access"
      |> example(func() {
        let apply = func(a, f) { f(a) }
        assert_equal(apply(person, func(x) { x.name }), "Quinn")
      }),
  ]
}

func shadowed_module_tests() {
  [
    "this module"
    |> example(func() {
...
func unicode_overflow_tests() {
  ...
  [
    "🌵 vs 5"
    |> example(func() { assert_equal(False, "🌵" == "5") }),
  ]
}
...
// https://github.com/gleam-lang/gleam/issues/1214
// https://github.com/gleam-lang/gleam/issues/1380
func tail_recursive_accumulate_down(x, y) {
  case x {
    0 -> y
...
func bool_negation_tests() {
  [
    "!True"
      |> example(func() { assert_equal(False, !True) }),
    "!False"
      |> example(func() { assert_equal(True, !False) }),
    // This would crash if the right hand side evaluated
    "!True && panic"
      |> example(func() { assert_equal(False, !True && panic) }),
    "!False || panic"
      |> example(func() { assert_equal(True, !False || panic) }),
  ]
}

func int_negation_tests() {
  [
    "-a"
      |> example(func() {
        let a = 3
...
func string_pattern_matching_tests() {
  [
...
@target(javascript)
func typescript_file_included_tests() {
  let path = "./build/dev/javascript/language/ffi_typescript.ts"
  [
    path
    |> example(func() { assert_equal(file_exists(path), True) }),
  ]
}
